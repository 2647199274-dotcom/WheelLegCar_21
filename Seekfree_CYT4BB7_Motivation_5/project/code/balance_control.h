...
// 陀螺仪数据转换（原始数据转换）
#define GYRO_DATA_X          (-imu660ra_gyro_y)      // 陀螺仪前后方向（前正后负）
#define GYRO_DATA_Y          (-imu660ra_gyro_x)      // 陀螺仪左右轮腿高度变化（左负右正）
#define GYRO_DATA_Z          (imu660ra_gyro_z)      // 陀螺仪方向（左负右正）
#define GYRO_TRANSITION_FACTOR   (16.384f)           // 陀螺仪数据转换系数（LSB/(°/s)）

// 加速度计数据转换
#define ACC_DATA_X           (imu660ra_acc_x)       // 加速度计左右轮腿高度变化（左负右正）
#define ACC_DATA_Y           (-imu660ra_acc_y)      // 加速度计前后方向（前正后负）
#define ACC_DATA_Z           (imu660ra_acc_z) // 加速度计垂直方向（减去初始偏移）
...
//------------------------------------------------------------------------------
void balance_cascade_init(void);	//最终参数版
void balance_cascade_init_test(void);	//调试测试版
