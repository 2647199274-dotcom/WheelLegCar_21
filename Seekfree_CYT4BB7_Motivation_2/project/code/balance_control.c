...
/*void balance_cascade_init(void)
{
    ...
    roll_balance_cascade.posture_value.mechanical_zero = -4.0f;     // 机械零点初始化为 -4°
    ...
    roll_balance_cascade.angle_cycle.i_value_pro   = 0.0f;    // 角度环积分比例
}*/
...
void balance_cascade_init(void)
{
    ...
    // 保存俯仰平衡控制参数到备份结构体
    memcpy(&pitch_balance_cascade_resave, &pitch_balance_cascade, sizeof(roll_balance_cascade_resave));
    // 初始化俯仰平衡控制的四元数模块
    quaternion_module_init(&pitch_balance_cascade);

    // 初始化横滚平衡控制的姿态参数
    roll_balance_cascade.posture_value.call_cycle      = 0.001;     // 调用周期 0.001s (1ms)
    roll_balance_cascade.posture_value.mechanical_zero = 0.0f;      // 机械零点初始化为 0
    ...
    // 保存横滚平衡控制初始参数到备份结构体
    memcpy(&roll_balance_cascade_resave, &roll_balance_cascade, sizeof(pitch_balance_cascade_resave));
    // 初始化横滚平衡控制的四元数模块
