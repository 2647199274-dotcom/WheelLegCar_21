...
//-----------------------------------------------------------------------------
void steer_duty_set(steer_control_struct *control_data, int16_t duty)
{
...
// 备注信息     根据偏移量控制舵机相对当前位置转动，偏移量正负由转向方向参数 steer_dir 决定，
//             最终位置限制在 -10000 ~ 10000 之间，仅在舵机使能状态下生效
//-----------------------------------------------------------------------------
void steer_control(steer_control_struct *control_data, int16_t move_num)
{
    if (control_data->steer_state)  // 判断舵机是否处于使能状态
    {
        // 根据转向方向计算新位置：steer_dir 为 1 时正向偏移，为 -1 时反向偏移
        control_data->now_location = control_data->now_location + 
                                    (control_data->steer_dir == 1 ? move_num : -move_num);
...
void steer_enable(steer_control_struct *control_data)
{
    ...
    pwm_set_duty(control_data->pwm_pin, control_data->now_location);  // 恢复到禁用前的最后位置
}
