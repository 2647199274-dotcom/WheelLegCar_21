...
void motor_control(int speed_left, int speed_right)//正数向前
{
    ...
    left_duty = speed_left;
    right_duty = speed_right;
    
    left_duty = (left_duty > 10000) ? 10000 : (left_duty < -10000) ? -10000 : left_duty;
    right_duty = (right_duty > 10000) ? 10000 : (right_duty < -10000) ? -10000 : right_duty;
    
    small_driver_set_duty(-left_duty, right_duty);
    
    //printf("%d,%d,%d,%d\n",control_data.speed_target_left,control_data.speed_FB_left,control_data.speed_target_right,control_data.speed_FB_right);//输出左右电机目标速度和实际速度
}
