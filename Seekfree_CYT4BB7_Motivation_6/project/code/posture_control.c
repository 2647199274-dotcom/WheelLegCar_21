#include "posture_control.h"
#include "turn.h"

// 外部控制标志
extern bool control_calc_flag;

//====================================================宏定义====================================================
#define WHEEL_CIRCUMFERENCE   (6.4f)    

//====================================================全局变量定义====================================================
int16 balance_duty_max = 3000;
int16 turn_duty_max    = 3000;
int16 jump_time_num[4] = {110, 110, 30, 100};  // 跳跃各阶段时间 起跳、收腿、准备缓冲、执行缓冲
int16 single_steer_change = 15;
//====================================================外部变量声明====================================================
extern small_device_value_struct motor_value;

//-----------------------------------------------------------------------------
// 函数简介     计算并更新车辆状态标志
// 返回参数     void
// 使用示例     car_state_calculate();
// 备注信息     根据横滚角、转向电流、实际距离等参数切换车辆悬挂状态和运行状态，同时处理 PID 参数的渐变
//-----------------------------------------------------------------------------
static bool data_change_flag = true;
void car_state_calculate(void)
{
    if (func_abs(pitch_balance_cascade.posture_value.pit) > 40.0f || func_abs(pitch_balance_cascade.posture_value.rol) > 40.0f)
    {
        jump_flag = 0;

        run_state = 0;                                                         // 停止运行

        pitch_balance_cascade.angular_speed_cycle.i_value = 0;                  // 重置角速度环 PID 积分值

        pitch_balance_cascade.angle_cycle.i_value = 0;
    }
    else
    {
        if (run_state == 0)
        {
            sys_times = 0;

            run_state = 1;
        }
    }

    // if (sys_times == 10000)
    // {
	// jump_flag = 1;
    // }

    
    if (jump_flag)
    {
      if(data_change_flag)
      {
	pitch_balance_cascade.angle_cycle.p = pitch_balance_cascade_resave.angle_cycle.p * 0.5f;
	pitch_balance_cascade.speed_cycle.p = pitch_balance_cascade_resave.speed_cycle.p * 0.5f;
	data_change_flag = false;
      }
	pitch_balance_cascade.angular_speed_cycle.i_value = 0;                          // 重置角速度环 PID 积分值
	pitch_balance_cascade.angle_cycle.i_value = 0;
    }
    
    if (sys_times < 500)                                                       // 系统计时小于 500 时，PID 参数从 20% 渐变到 100%
    {
	// 角度环 P 参数渐变 (0.2 到 1.0 倍原始值)
	pitch_balance_cascade.angle_cycle.p = pitch_balance_cascade_resave.angle_cycle.p * (0.2 + (float)sys_times / 500.0f * 0.8f);
	// 速度环 P 参数渐变 (0.2 到 1.0 倍原始值)
	pitch_balance_cascade.speed_cycle.p = pitch_balance_cascade_resave.speed_cycle.p * (0.2 + (float)sys_times / 500.0f * 0.8f);

	pitch_balance_cascade.angle_cycle.i_value = 0;                          // 重置角度环积分值
    }
    else                                                                      // 系统计时大于等于 500 时，使用原始 PID 参数
    {
      if(data_change_flag)
      {
	pitch_balance_cascade.angle_cycle.p = pitch_balance_cascade_resave.angle_cycle.p;  // 恢复角度环 P 参数
	pitch_balance_cascade.speed_cycle.p = pitch_balance_cascade_resave.speed_cycle.p;  // 恢复速度环 P 参数
	
	data_change_flag = false;
      }
    }
}

//-----------------------------------------------------------------------------
// 函数简介     车辆舵机控制
// 返回参数     void
// 使用示例     car_steer_control();
// 备注信息     控制跳跃、左右倾斜、腿部前后倾斜、自动复位等操作
//-----------------------------------------------------------------------------

void car_steer_control(void)
{
    int16 steer_location_offset[4] = {0};
    int16 steer_target_offset[4] = {0};


    static float steer_balance_angle_count = 0;
    static float steer_output_duty_filter = 0;

    int16 steer_output_duty = 0;
    float steer_balance_angle = 0;

    float pitch_offset = (30.0f - func_limit_ab(func_abs(pitch_balance_cascade.posture_value.pit + pitch_balance_cascade.posture_value.mechanical_zero), 0.0f, 30.0f)) / 30.0f;

    steer_output_duty = func_limit_ab((int16)(pitch_balance_cascade.speed_cycle.out / 7.0f), -250, 250) * 6;
    steer_output_duty = (int16)((float)steer_output_duty * pitch_offset);

    steer_output_duty_filter = (steer_output_duty_filter * 19 + (float)steer_output_duty) / 20.0f;

    if (jump_flag == 0)
    {
        if (sys_times < 2000)
        {
            steer_balance_angle = 0;
            roll_balance_cascade.angle_cycle.i_value = 0;
        }
        else
        {
            steer_balance_angle = func_limit_ab(roll_balance_cascade.angle_cycle.out, -450, 450) * 6;
        }
    }

    steer_balance_angle_count = steer_balance_angle;

    steer_location_offset[0] = (steer_1.now_location - steer_1.center_num) * steer_1.steer_dir;
    steer_location_offset[1] = (steer_2.now_location - steer_2.center_num) * steer_2.steer_dir;
    steer_location_offset[2] = (steer_3.now_location - steer_3.center_num) * steer_3.steer_dir;
    steer_location_offset[3] = (steer_4.now_location - steer_4.center_num) * steer_4.steer_dir;

    steer_target_offset[0] = (int16)( steer_output_duty_filter - (steer_balance_angle_count > 0 ? steer_balance_angle_count : 0));
    steer_target_offset[1] = (int16)( steer_output_duty_filter + (steer_balance_angle_count < 0 ? steer_balance_angle_count : 0));
    steer_target_offset[2] = (int16)(-steer_output_duty_filter - (steer_balance_angle_count > 0 ? steer_balance_angle_count : 0));
    steer_target_offset[3] = (int16)(-steer_output_duty_filter + (steer_balance_angle_count < 0 ? steer_balance_angle_count : 0));

    if (run_state == 1)
    {
        if (jump_flag == 0)
        {
            steer_control(&steer_1, func_limit_ab(steer_target_offset[0] - steer_location_offset[0], -single_steer_change, single_steer_change));
            steer_control(&steer_2, func_limit_ab(steer_target_offset[1] - steer_location_offset[1], -single_steer_change, single_steer_change));
            steer_control(&steer_3, func_limit_ab(steer_target_offset[2] - steer_location_offset[2], -single_steer_change, single_steer_change));
            steer_control(&steer_4, func_limit_ab(steer_target_offset[3] - steer_location_offset[3], -single_steer_change, single_steer_change));
        }
        else
	{
	    jump_time++;

	    if (jump_time < jump_time_num[0])  // 起跳
	    {
		jump_flag = 1;
		steer_duty_set(&steer_1, steer_1.center_num + 2500);
		steer_duty_set(&steer_2, steer_2.center_num - 2500);
		steer_duty_set(&steer_3, steer_3.center_num - 2500);
		steer_duty_set(&steer_4, steer_4.center_num + 2500);
	    }
	    else if (jump_time < (jump_time_num[0] + jump_time_num[1]))  // 收脚
	    {
		jump_flag = 2;
		steer_duty_set(&steer_1, steer_1.center_num);
		steer_duty_set(&steer_2, steer_2.center_num);
		steer_duty_set(&steer_3, steer_3.center_num);
		steer_duty_set(&steer_4, steer_4.center_num);
	    }
	    else if (jump_time < (jump_time_num[0] + jump_time_num[1] + jump_time_num[2]))  // 预备缓冲
	    {
		jump_flag = 3;
		steer_duty_set(&steer_1, steer_1.center_num + 1400);
		steer_duty_set(&steer_2, steer_2.center_num - 1400);
		steer_duty_set(&steer_3, steer_3.center_num - 1400);
	        steer_duty_set(&steer_4, steer_4.center_num + 1400);
	    }
	    else if (jump_time < (jump_time_num[0] + jump_time_num[1] + jump_time_num[2] + jump_time_num[3]))  // 执行缓冲
	    {
		jump_flag = 4;
		steer_control(&steer_1, -14);
		steer_control(&steer_2, -14);
		steer_control(&steer_3, -14);
		steer_control(&steer_4, -14);
	    }
	    else
	    {
		jump_flag = 0;
		jump_time = 0;
	    }
	}
    }	
    else
    {
	steer_control(&steer_1, func_limit_ab(steer_1.center_num - steer_1.now_location, -1, 1) * steer_1.steer_dir);
	steer_control(&steer_2, func_limit_ab(steer_2.center_num - steer_2.now_location, -1, 1) * steer_2.steer_dir);
	steer_control(&steer_3, func_limit_ab(steer_3.center_num - steer_3.now_location, -1, 1) * steer_3.steer_dir);
	steer_control(&steer_4, func_limit_ab(steer_4.center_num - steer_4.now_location, -1, 1) * steer_4.steer_dir);
    }
}

//-----------------------------------------------------------------------------
// 函数简介     车辆电机占空比控制
// 返回参数     void
// 使用示例     car_motor_control();
// 备注信息     根据运行标志控制左右电机的占空比，通过限幅函数限制输出范围，最终设置电机驱动
//-----------------------------------------------------------------------------
void car_motor_control(void)
{
    car_distance += ((float)car_speed / 60.0f * WHEEL_CIRCUMFERENCE * PI * 0.001f);

    if (run_state)                                                         // 当运行状态为 1 时，计算电机占空比
    {
        
        float base_duty = (float)pitch_balance_cascade.angular_speed_cycle.out;
        float turn_output = turn_control_calculate();
        
        left_motor_duty  = func_limit_ab((int16)(base_duty - turn_output), -balance_duty_max, balance_duty_max);  // 左电机占空比
        right_motor_duty = func_limit_ab((int16)(base_duty + turn_output), -balance_duty_max, balance_duty_max);  // 右电机占空比

//        left_motor_duty  = func_limit_ab(left_motor_duty  + GYRO_DATA_Z / 3,  -turn_duty_max,  turn_duty_max );
//        right_motor_duty = func_limit_ab(right_motor_duty - GYRO_DATA_Z / 3,  -turn_duty_max,  turn_duty_max );
    }
    else                                                                  // 当运行状态为 0 时，电机关闭
    {
        left_motor_duty  = 0;                                              // 左电机占空比设为 0
        right_motor_duty = 0;                                              // 右电机占空比设为 0
    }

    small_driver_set_duty(-left_motor_duty, right_motor_duty);              // 设置驱动的电机占空比
}

//-----------------------------------------------------------------------------
// 函数简介     PIT 定时器中断回调函数，处理定时任务
// 返回参数     void
// 使用示例     由 PIT 中断触发调用，无需手动调用
// 备注信息     周期性执行传感器数据采集、状态计算、PID 控制、电机控制等任务，各子任务按不同周期执行
//-----------------------------------------------------------------------------

void pit_callback(void)
{
    sys_times++;                                                         // 系统计时自增

    for (int i = 0; i < 20; i++)
    {
        system_time_state[i] = (sys_times % (i + 1) == 0 ? 1 : system_time_state[i]);
    }

//    imu660ra_get_gyro();                                                  // 获取 IMU660RA 陀螺仪数据
//    imu660ra_get_acc();                                                  // 获取 IMU660RA 加速度计数据
//    quaternion_module_calculate(&pitch_balance_cascade);                   // 计算四元数，更新姿态数据
    
    Attitude_get();
    Attitude_update(&pitch_balance_cascade);
    Attitude_update(&roll_balance_cascade);
    
    if (sys_times % 10 == 0)                                             // 每 10 个周期执行一次（约 10 * 中断周期）
    {
        car_speed = (motor_value.receive_left_speed_data - motor_value.receive_right_speed_data) / 2;   // 计算车辆速度：左右电机速度差的一半

        pid_control(&pitch_balance_cascade.speed_cycle, 0.0f, (float)car_speed);  // 速度环 PID 控制，目标值为 0.0f（原地保持平衡）
    }

    if (sys_times % 3 == 0)                                              // 每 5 个周期执行一次
    {
        // 角度环 PID 控制，目标值为速度环输出减去机械零点
        pid_control(&pitch_balance_cascade.angle_cycle, 0.0 - pitch_balance_cascade.posture_value.mechanical_zero, pitch_balance_cascade.posture_value.pit);
        // 角度环 PID 控制，目标值为速度环输出减去机械零点
        pid_control(&roll_balance_cascade.angle_cycle, 0.0f - roll_balance_cascade.posture_value.mechanical_zero, roll_balance_cascade.posture_value.rol);
    }

    // 角速度环 PID 控制，目标值为角度环输出，当前值为前后方向陀螺仪数据
    pid_control(&pitch_balance_cascade.angular_speed_cycle, pitch_balance_cascade.angle_cycle.out, GYRO_DATA_Y);

    // car_state_calculate();  // 检测车辆状态
    // car_steer_control();    // 车辆舵机控制
    // car_motor_control();    // 车辆电机控制
    control_calc_flag = true;
    
}













