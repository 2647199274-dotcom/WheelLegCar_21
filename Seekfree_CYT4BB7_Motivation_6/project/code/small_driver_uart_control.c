...
//PID_Positional_Data motor_pid_left;
//PID_Positional_Data motor_pid_right;
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     无刷驱动 串口接收回调函数
// 参数说明     void
// 返回参数     void
...
{
//    PID_Positional_Init(&motor_pid_left, 8.0f, 2.0f, 0.0f, 0.005f, 5000.0f, -5000.0f, 5000.0f, 0.0f);
//    PID_Positional_Init(&motor_pid_right, 8.0f, 2.0f, 0.0f, 0.005f, 5000.0f, -5000.0f, 5000.0f, 0.0f); 
} 

move_filter_struct motor_left_speed;
move_filter_struct motor_right_speed;

void motor_filter_init(void)
{
  move_filter_init(&motor_left_speed);
    
  move_filter_init(&motor_right_speed);
}

void motor_control(int speed_left, int speed_right)
{
//    control_data.speed_target_left = speed_left; 
//    control_data.speed_target_right = speed_right; 
//    control_data.speed_FB_left = motor_value.receive_left_speed_data;
//    control_data.speed_FB_right = motor_value.receive_right_speed_data;
//    
    int16 left_duty = speed_left;
    int16 right_duty = speed_right;

//    left_duty = (int16)(PID_Positional_Calculate(&motor_pid_left, speed_left, control_data.speed_FB_left) + 0.5f);
//    right_duty = (int16)(PID_Positional_Calculate(&motor_pid_right, speed_right, control_data.speed_FB_right) + 0.5f);
//   
    
    if(left_duty > 5000)   left_duty = 5000;
    if(left_duty < -5000)  left_duty = -5000;
    if(right_duty > 5000)  right_duty = 5000;
    if(right_duty < -5000) right_duty = -5000;
    
    small_driver_set_duty(-left_duty,right_duty);
//    
//    move_filter_calc(&motor_left_speed,speed_left);
//    move_filter_calc(&motor_right_speed,speed_right);
    
//    small_driver_set_duty(-motor_left_speed.data_average, motor_right_speed.data_average);
