...
void car_motor_control(void)
{
    ...
    {
        left_motor_duty  = func_limit_ab((int16)pitch_balance_cascade.angular_speed_cycle.out, -balance_duty_max, balance_duty_max);  // 左电机占空比：取角速度环输出，限幅
        right_motor_duty = func_limit_ab((int16)pitch_balance_cascade.angular_speed_cycle.out, -balance_duty_max, balance_duty_max);  // 右电机占空比：取角速度环输出，限幅
...
//       small_driver_get_speed();
    ...
    if (sys_times % 3 == 0)                                              // 每 5 个周期执行一次
    {
        ...
        pid_control(&pitch_balance_cascade.angle_cycle, pitch_balance_cascade.speed_cycle.out - pitch_balance_cascade.posture_value.mechanical_zero, pitch_balance_cascade.posture_value.pit);
        // 角度环 PID 控制，目标值为速度环输出减去机械零点
